...
namespace WereViewApp.Helpers {
    ...
    public static class HtmlHelpers {
        ...
        public static HtmlString DropDowns(this HtmlHelper helper, string valueField, string textField, string htmlName = null, string displayName = null, string modelValue = null, string isRequried = "*", string classes = null, string toolTipValue = null, string otherAttributes = "", string tableName = null, WereViewApp.AppVar.ConnectionStringType connectionType = AppVar.ConnectionStringType.DefaultConnection) {
        ...
        public static string Truncate(this HtmlHelper helper, string input, int starting, int length) {
            ...
            return input.Substring(starting, (int)length);
        ...
        public static string GetCurrentURLString(this HtmlHelper helper) {
            return System.Web.HttpContext.Current.Request.RawUrl.ToString();
        }
        /// <summary>
        /// 
        /// </summary>
        ...
        /// <returns></returns>
        public static string UploadedImageSrc(this HtmlHelper helper, string addtionalRootPath, IUploadableFile file, bool isPrivate = false, bool asTemp = false, string tempString = "_temp", string rootPath = "~/Uploads/Images/") {
            if (isPrivate) {
                rootPath += "Private/";
            }
            ...
            string fileName = file.UploadGuid + "-" + file.Sequence.ToString() + tempString + "." + file.Extension;

            string path = string.Format("{0}{1}", rootPath, fileName);
            return AppVar.Url + VirtualPathUtility.ToAbsolute(path);
            //return (markup);
        }

        public static string GetOrganizeName(IUploadableFile file, bool includeExtention = false, bool asTemp = false, string tempString = "_temp") {
            var ext = "";
            if (!asTemp) {
                tempString = "";
            }
        ...
        public static HtmlString ImageFromAbsolutePath(this HtmlHelper helper, string src, string alt) {
            string markup = string.Format("<img src='{0}' alt='{1}'/>", src, alt);
            return new HtmlString(markup);
            //return (markup);
        }
        ...
        public static HtmlString Image(this HtmlHelper helper, string src, string alt) {
            string markup = string.Format("<img src='{0}' alt='{1}'/>", VirtualPathUtility.ToAbsolute(src), alt);
            return new HtmlString(markup);
            //return (markup);
        }
        /// <summary>
        ...
        public static HtmlString Image(this HtmlHelper helper, string folder, string src, string ext, string alt) {
            string markup = string.Format("<img src='{0}{1}.{2}' alt='{3}'/>", VirtualPathUtility.ToAbsolute(folder), src, ext, alt);
            //return  new HtmlString(markup);
            return new HtmlString(markup);
        }
