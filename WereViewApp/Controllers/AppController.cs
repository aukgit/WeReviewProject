...
namespace WereViewApp.Controllers {
    ...
    public class AppController : AdvanceController {
        ...
        const string _controllerName = "App";
        ...
        string PostedSuccessFully(string title, bool published) {
            if (!published) {
                return "Your new app '" + title + "' has been saved successfully but not published. You can also save a new one right away.";
            } else {
                return "Your new app '" + title + "' has been successfully saved and published. You can also save a new one right away.";
            }
        }

        string EditSuccessFully(string title, bool published) {
            if (!published) {
        ...
        string EditFailed(string title) {
            return "Your app '" + title + "' has been failed to modify.";
        }
        string PostedFailed(string title) {
        ...
        public ActionResult SingleAppDisplay(string platform, float platformVersion, string category, string url) {
            var app = algorithms.GetSingleAppForDisplay(platform, platformVersion, category, url, 30, db);
            if (app != null) {
                algorithms.IncreaseViewCount(app,db);
                return View(app);
            }
        ...
        [HttpGet]
        public JsonResult DeleteGalleryImage(Guid uploadGuid, byte sequence, string __RequestVerificationToken) {
            ...
            if (gallery != null) {
                ...
                var absPath = WereViewStatics.uProcessorGallery.VirtualPathtoAbsoluteServerPath(WereViewStatics.uProcessorGallery.GetCombinationOfRootAndAdditionalRoot() + fileName);
                if (System.IO.File.Exists(absPath)) {
                    System.IO.File.Delete(absPath);
                }

                absPath = WereViewStatics.uProcessorGallery.VirtualPathtoAbsoluteServerPath(WereViewStatics.uProcessorGallery.GetCombinationOfRootAndAdditionalRoot() + fileName);
                if (System.IO.File.Exists(absPath)) {
                    System.IO.File.Delete(absPath);
                }
        ...
        public ActionResult EditGalleryUploads(App app) {
            ...
            var path = AppVar.Url + WereViewStatics.uProcessorGallery.RootPath.Replace("~/", "/") + WereViewStatics.uProcessorGallery.AdditionalRoots;
        ...
        int GetAppGalleryImageCountValueStatic(Guid UploadGuid) {
            if (UploadGuid == null) {
                return 0;
            }
            string sessionName = UploadGuid.ToString() + "-staticCount";
            ...
            if (sessionCount != null) {
                ...
                int count = db.Galleries.Count(n => n.UploadGuid == UploadGuid && n.GalleryCategoryID == GalleryCategoryIDs.AppPageGallery);
        ...
        bool ViewTapping(ViewStates view, App app = null) {
            switch (view) {
        ...
        /// <returns>If successfully saved returns true or else false.</returns>
        bool SaveDatabase(ViewStates view, IApp app = null) {
        ...
        public async Task<JsonResult> SaveDraft(App app) {
            long userId = UserManager.GetLoggedUserId();
            if (app.AppID != 0) {
        ...
        /// <returns></returns>
        App GetAppfromDraft(AppDraft appDraft) {
            ...
            rApp.PostedByUserID = (long)appDraft.PostedByUserID;
        ...
        AppDraft GetDraft(App app, AppDraft appDraft = null) {
            if (appDraft == null) {
            ...
            new Thread(() => {
                if (!string.IsNullOrWhiteSpace(tagString)) {
                    using (var db2 = new WereViewAppEntities()) {
                        string[] tags = tagString.Split(";,".ToCharArray());
        ...
        string fixScriptTag(string str) {
            if (!string.IsNullOrWhiteSpace(str)) {
                //|embed|object|frameset|frame|iframe|
                string regEx = @"</?(?i:script|meta|link|style)(.|\n|\s)*?>";
        ...
        private void AddNecessaryWhenModified(App app) {
            // Set userid, last mod, save virtual fields, fix iframe, URL
            AddNecessaryBothOnPostingNEditing(app);
            algorithms.RemoveSingleAppFromCacheOfStatic(app, db);
        ...
        private void AddNecessaryBothOnPostingNEditing(App app) {
            app.URL = algorithms.GenerateURLValid(app.PlatformVersion, app.CategoryID, app.AppName, app.PlatformID, db, app.AppID);
            app.URLWithoutEscapseSequence = algorithms.GetURLStringExceptEscapeSequence(app.URL);
        ...
        public ActionResult Details(System.Int64 id) {
            ...
            bool viewOf = ViewTapping(ViewStates.Details, app);
            return View(app);
        }
        #endregion
        ...
        public void RemoveTempUploadRelatedtoApp(Guid uploadGuid) {
            new Thread(() => {
                using (var db2 = new WereViewAppEntities()) {
                    db2.Database.ExecuteSqlCommand("DELETE FROM TempUpload WHERE RelatingUploadGuidForDelete = @p0", uploadGuid);
                }
            }).Start();
        }
        ...
        public ActionResult Post(App app) {
            bool viewOf = ViewTapping(ViewStates.CreatePost, app);
            ...
            if (ModelState.IsValid) {
                db.Apps.Add(app);
                bool state = SaveDatabase(ViewStates.Create, app);
                if (state) {
                    ManageTagsInDatabase(app.AppID, app.UploadGuid, app.Tags);
                    RemoveTempUploadAndDraftFromRelatedApp(app.UploadGuid);
                    AppVar.SetSavedStatus(ViewBag, PostedSuccessFully(app.AppName, app.IsPublished)); // Saved Successfully.
                    app.UploadGuid = Guid.NewGuid(); // new post can be made
        ...
        byte GetSequenceNumber(Guid UploadGuid) {
            if (UploadGuid == null) {
                return 0;
            }
            var sessionCount = Session[UploadGuid.ToString()];
            Session[UploadGuid.ToString() + "-staticCount"] = null;
            if (sessionCount != null) {
                var b = byte.Parse(sessionCount.ToString());
                Session[UploadGuid.ToString()] = b + (byte)1;
                return b;
            } else {
                //try to get from the database.
                int max = 0;
                if (db.Galleries.Any(n => n.UploadGuid == UploadGuid && n.GalleryCategoryID == GalleryCategoryIDs.AppPageGallery)) {
                    max = db.Galleries.Where(n => n.UploadGuid == UploadGuid && n.GalleryCategoryID == GalleryCategoryIDs.AppPageGallery).Max(n => n.Sequence);
                }
        ...
        int GetHowManyGalleryImageExist(Guid UploadGuid) {
            if (UploadGuid == null) {
                return 0;
            }
            string sessionName = UploadGuid.ToString() + "-count";
            Session[UploadGuid.ToString() + "-staticCount"] = null;
            var sessionCount = Session[sessionName];
            if (sessionCount != null) {
                var b = byte.Parse(sessionCount.ToString());
                ...
                max = db.Galleries.Count(n => n.UploadGuid == UploadGuid && n.GalleryCategoryID == GalleryCategoryIDs.AppPageGallery);
        ...
        void resetSessionForUploadSequence(Guid UploadGuid) {
            Session[UploadGuid.ToString()] = null;
            Session[UploadGuid.ToString() + "-count"] = null;
            Session[UploadGuid.ToString() + "-staticCount"] = null;
        }
        ...
        public async Task<JsonResult> UploadGallery(App app, IEnumerable<HttpPostedFileBase> Galleries) {
            if (Galleries != null && app.UploadGuid != null) {
                ...
                if (nextCount > AppVar.Setting.GalleryMaxPictures) {
                    ...
                    return Json(new { isUploaded = false, uploadedFiles = 0, message = "You are out of your limit." }, "text/html");
                }
                string fileName = app.UploadGuid.ToString();
                bool firstTime = true;
                int countDone = 0;
                foreach (var file in Galleries) {
                    if (file.ContentLength > 0) {
                        ...
                        if (!firstTime) {
                            ...
                            if (nextCount > AppVar.Setting.GalleryMaxPictures) {
                                resetSessionForUploadSequence(app.UploadGuid);
                                return Json(new { isUploaded = true, uploadedFiles = countDone, message = "You are out of your limit." }, "text/html");
                            }
                        } else {
                            firstTime = false;
                        }
                        ...
                        Gallery gallery = await db.Galleries.FirstOrDefaultAsync(n => n.UploadGuid == app.UploadGuid && n.GalleryCategoryID == galleryCategory.GalleryCategoryID && n.Sequence == nextSequence);
                        ...
                        if (gallery == null) {
                            ...
                            // information already exist in the database no need to save.
                        }

                        // resize
                        //new Thread(() => {
                        WereViewStatics.uProcessorGallery.ProcessImage(gallery, galleryCategory);
                        string source = "~/Uploads/Images/" + CommonVars.ADDITIONAL_ROOT_GALLERY_LOCATION + UploadProcessor.GetOrganizeNameStatic(gallery, true, true);
                        string target = "~/Uploads/Images/" + CommonVars.ADDITIONAL_ROOT_GALLERY_ICON_LOCATION + UploadProcessor.GetOrganizeNameStatic(gallery, true);
                        WereViewStatics.uProcessorGallery.ProcessImage(source, target, thumbsCategory.Width, thumbsCategory.Height, gallery.Extension);
                        if (System.IO.File.Exists(WereViewStatics.uProcessorGallery.VirtualPathtoAbsoluteServerPath(target))) {
                ...
                return Json(new { isUploaded = true, uploadedFiles = countUploaded, message = "+" + countUploaded + " files successfully done." }, "text/html");
            }
        ...
        public JsonResult ProcessSingleUploads(App app, HttpPostedFileBase file, int GalleryId, UploadProcessor uploadProcessorSepecific) {
            if (file != null && app.UploadGuid != null) {
                ...
                if (file.ContentLength > 0) {
                    ...
                    Gallery gallery = db.Galleries.FirstOrDefault(n => n.UploadGuid == app.UploadGuid && n.GalleryCategoryID == galleryCategory.GalleryCategoryID);
                    if (gallery == null) {
                        gallery = new Gallery();
                        gallery.GalleryID = Guid.NewGuid();
                        gallery.UploadGuid = app.UploadGuid;
        ...
        public JsonResult UploadHomeFeatured(App app, HttpPostedFileBase HomePageFeatured) {
            return ProcessSingleUploads(app, HomePageFeatured, GalleryCategoryIDs.HomePageFeatured, WereViewStatics.uProcessorHomeFeatured);
        }
        #endregion
        ...
        public JsonResult UploadHomePageIcon(App app, HttpPostedFileBase HomePageIcon) {
            return ProcessSingleUploads(app, HomePageIcon, GalleryCategoryIDs.HomePageIcon, WereViewStatics.uProcessorHomeIcons);
        }
        #endregion
        ...
        public JsonResult UploadSearchIcon(App app, HttpPostedFileBase SearchIcon) {
            return ProcessSingleUploads(app, SearchIcon, GalleryCategoryIDs.SearchIcon, WereViewStatics.uProcessorSearchIcons);
        }
        #endregion
        ...
        public JsonResult UploadSuggestionIcon(App app, HttpPostedFileBase SuggestionIcon) {
            return ProcessSingleUploads(app, SuggestionIcon, GalleryCategoryIDs.SuggestionIcon, WereViewStatics.uProcessorSuggestionIcons);
        }
        #endregion
        ...
        public ActionResult Edit(System.Int64 id) {
            ...
            bool viewOf = ViewTapping(ViewStates.Edit, app);
        ...
        public ActionResult Edit(App app) {
            bool viewOf = ViewTapping(ViewStates.EditPost, app);
            if (ModelState.IsValid) {
                AddNecessaryWhenModified(app);
                db.Entry(app).State = EntityState.Modified;
                bool state = SaveDatabase(ViewStates.Edit, app);
                if (state) {
                    ManageTagsInDatabase(app.AppID, app.UploadGuid, app.Tags);
